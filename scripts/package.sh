...
for target in windows/amd64 windows/arm64 darwin/amd64 darwin/arm64; do
    ...
    if [ "$os" = windows ]; then
        ...
        (cd "$bundle" && zip -q "$root/dist/ghi_${version}_${os}_${arch}.zip" "ghi$suffix" ghi.sha256 install.ps1)
    else
        cp install/install.sh "$bundle/"
        tar -czf "$root/dist/ghi_${version}_${os}_${arch}.tar.gz" -C "$bundle" ghi ghi.sha256 install.sh
    fi
